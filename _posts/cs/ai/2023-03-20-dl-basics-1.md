...
- 손실 함수?
    ...
    - 분류<sub>Classification</sub>
        ...
        - 과연 cross enropy loss가 분류 문제를 푸는 데 최적일까?
        - $y_i^{(d)}$는 한 개만 1, 나머지는 0이어서 해당 클래스의 $\hat y_i^{(d)}$가 다른 것들보다 크기만 하면 됨. 즉 $\hat y_i^{(d)}$ 간의 상대적인 관계를 따지는 것.
    - 확률 기반<sub>Probabilistic</sub>
