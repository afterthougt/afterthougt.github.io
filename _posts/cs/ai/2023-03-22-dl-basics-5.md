...
- As the first term does not depend on $$P_\theta$$, minimizing the KL-divergence is equivalent to maximizing the expected log-likelihood.
    ...
    $$
    \begin{aligned}\argmin_{P_\theta}\mathcal D(P_\text{data}\Vert P_\theta)&=\argmin_{P_\theta}-\mathbb E_{\mathbf x\sim P_\text{data}}[\log P_\theta(\mathbf x)]\\&=\argmax_{P_\theta}\mathbb E_{\mathbf x\sim P_\text{data}}[\log P_\theta(\mathbf x)]\end{aligned}
    $$
